...
generate.bootstrap.plots.for.transcriptome <- function(sct_data,tt,thresh,annotLevel=1,reps,full_results=NA,listFileName="",showGNameThresh=25){
    ...
    for(dirS in c("Up","Down")){
        ...
        mouse.hits = mouse.hits[mouse.hits %in% rownames(sct_data[[1]]$cell_dists)]
        mouse.bg = unique(tt$MGI.symbol)
        mouse.bg = mouse.bg[!mouse.bg %in% mouse.hits]
        mouse.bg = mouse.bg[mouse.bg %in% rownames(sct_data[[1]]$cell_dists)]
        ...
        for(s in 1:nReps){
            ...
            ValidGenes = rownames(sct_data[[annotLevel]]$cell_dists)[rownames(sct_data[[annotLevel]]$cell_dists) %in% bootstrap_set]

            expD = sct_data[[annotLevel]]$cell_dists[ValidGenes,]
        ...
        hit.exp = sct_data[[annotLevel]]$cell_dists[mouse.hits,]	#cell.list.exp(mouse.hits)
